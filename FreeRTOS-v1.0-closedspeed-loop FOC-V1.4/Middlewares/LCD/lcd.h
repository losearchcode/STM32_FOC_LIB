...
extern uint8_t lcd_flag[5][5];

extern uint8_t LCD_refresh_flag;

typedef struct {
    long LED1_Delay_time_Show;
    uint32_t xElapsedTime_Show;
		float AS5600_angle_Show;
		float zero_electric_angle_Show;
		float xCurrent_electricalAngle_Show;
		float	xCurrent_Vel_Show;
		float	PID_Error_Show;
} LCD_Show_t;

extern LCD_Show_t LCD_Show_Parameter;
...
void LCD_ShowIntNum(uint16_t x,uint16_t y,long num,uint8_t len,uint16_t fc,uint16_t bc,uint8_t sizey);//显示整数变量
