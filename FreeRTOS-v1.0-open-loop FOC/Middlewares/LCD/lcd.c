...
//	uint8_t* param = (uint8_t*)parameter;
	...
	LCD_ShowString(0,60,"V_MIN:",RED,WHITE,24,0);
	LCD_ShowString(0,90,"V_MAX:",RED,WHITE,24,0);
	LCD_ShowString(0,120,"D:",RED,WHITE,24,0);

}

/******************************************************************************
      函数说明：LCD_Show_Static动态显示
      入口数据：无   
      返回值：  无
******************************************************************************/
void LCD_Show_Dynamic(void* parameter)
{
	...
	uint8_t LED_Hz_level =(uint8_t)	param -> LED1_Delay_time_t;
	LCD_ShowIntNum(120,120,(uint8_t)(LED_Hz_level),4,RED,WHITE,24);
	
	uint32_t xCurrentTime_t =	param -> xCurrentTime_t;
	LCD_ShowIntNum(140,150,(uint32_t)(xCurrentTime_t),4,RED,WHITE,24);
