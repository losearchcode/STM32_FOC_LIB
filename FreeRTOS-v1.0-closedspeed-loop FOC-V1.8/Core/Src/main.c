...
//	float Vel_;
	...
	FOC_Para_M[0].Motor_State_.controller = Type_angle;				
...
//		adc1_ch1_ = _readADCVoltageInline(FOC_Para_M[0].Current_Param_.pinA);
//		adc1_ch2_ = _readADCVoltageInline(FOC_Para_M[0].Current_Param_.pinB);
		...
		{}
		#else
		adc1_ch1_ = ADC_Get_Average_VoltageInline(&hadc1,1);
		adc1_ch2_ = ADC_Get_Average_VoltageInline(&hadc1,2);
		if(Timecount>=20)
		{
//			printf("ADC_ch%d: %.3f  V\n",FOC_Para_M[0].Current_Param_.pinA.channel,(float)adc1_ch1_);
//			printf("ADC_ch%d: %.3f  V\n\n",FOC_Para_M[0].Current_Param_.pinB.channel,(float)adc1_ch2_);
