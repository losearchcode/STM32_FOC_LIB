...
/******************************************************************************/
void Motor_init(uint8_t Motor_No,TIM_HandleTypeDef* htimn)
{
	LPF_init(Motor_No);
...
//	voltage_limit = new_voltage_limit < voltage_limit ? new_voltage_limit : voltage_limit;
	...
	FOC_Para_M[Motor_No].Motor_Properties_.TimHandle = htimn;
	//V，电源电压
...
void Motor_initFOC(uint8_t Motor_No,float zero_electric_offset, Direction _sensor_direction)
{
	...
	printf("First get angle : %0.3f\r\n",FOC_Para_M[Motor_No].MagneticSensor_.Angle_Prev);
	vTaskDelay(50);
	...
	if(FOC_Para_M[Motor_No].Motor_State_.controller==Type_angle) FOC_Para_M[Motor_No].Motor_State_.target=FOC_Para_M[Motor_No].Motor_State_.shaft_angle;//角度模式，以当前的角度为目标角度，进入主循环后电机静止
...
{
	if( FOC_Para_M[Motor_No].Motor_State_.controller!=Type_angle_openloop && FOC_Para_M[Motor_No].Motor_State_.controller!=Type_velocity_openloop )
...
float velocityOpenloop(uint8_t Motor_No,float target_velocity)
{
	...
	Ts = (float)Get_Interval_Timetick(&FOC_Para_M[Motor_No].Open_Loop_Param_.Open_Loop_Timetick)*Tick_to_Time_Scale;
