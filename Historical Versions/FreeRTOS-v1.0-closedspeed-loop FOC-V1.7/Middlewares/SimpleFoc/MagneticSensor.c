...
/******************************************************************************/
unsigned short I2C_getRawCount(uint8_t Motor_No)
{
...
float getAngle(uint8_t Motor_No)
{
...
#if M1_AS5600
	angle_data = I2C_getRawCount(Motor_No);
#elif M1_TLE5012B
	...
	if(fabs(d_angle) > (0.8*FOC_Para_M[Motor_No].MagneticSensor_.Cpr) ) FOC_Para_M[Motor_No].MagneticSensor_.Full_Rotation_Offset += d_angle > 0 ? -_2PI : _2PI; 
	// save the current angle value for the next steps
	// in order to know if overflow happened
	FOC_Para_M[Motor_No].MagneticSensor_.Angle_Data_Prev = angle_data;
	// return the full angle 
	// (number of full rotations)*2PI + current sensor angle 
	return  (FOC_Para_M[Motor_No].MagneticSensor_.Full_Rotation_Offset + ( angle_data / (float)FOC_Para_M[Motor_No].MagneticSensor_.Cpr) * _2PI) ;
}
/******************************************************************************/
...
float getVelocity(uint8_t Motor_No)
{
	...
	Ts = (float)Get_Interval_Timetick(&FOC_Para_M[Motor_No].MagneticSensor_.AS5600_Vel_Timecount)/1e-3f;
