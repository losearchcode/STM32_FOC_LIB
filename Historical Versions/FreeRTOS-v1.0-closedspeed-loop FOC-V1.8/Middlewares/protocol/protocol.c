...
/**
  ...
  * @retval 无
  */
void set_p_i_d_vel(float p, float i, float d)
{
  	FOC_Para_M[0].PID_Vel_.P = p;
		FOC_Para_M[0].PID_Vel_.I = i;
		FOC_Para_M[0].PID_Vel_.D = d;
}

void set_p_i_d_angle(float p, float i, float d)
{
  	FOC_Para_M[0].PID_Angle_.P = p;
		FOC_Para_M[0].PID_Angle_.I = i;
		FOC_Para_M[0].PID_Angle_.D = d;
}
...
//        set_p_i_d_vel(p_temp, i_temp, d_temp);    // 设置 P I D
				
				set_p_i_d_angle(p_temp, i_temp, d_temp);
      }
      break;
...
void set_computer_value(uint8_t cmd, uint8_t ch, void *data, uint8_t num)
{
  ...
  HAL_UART_Transmit(&Protocol_UartHandle, (uint8_t *)&set_packet, sizeof(set_packet), 0xFFFFF);    // 发送数据头
  HAL_UART_Transmit(&Protocol_UartHandle, (uint8_t *)data, num, 0xFFFFF);                          // 发送参数
  HAL_UART_Transmit(&Protocol_UartHandle, (uint8_t *)&sum, sizeof(sum), 0xFFFFF);                  // 发送校验和
}
...
void time_period_fun(void)
{
	...
	if(Pro_Motor_EN)
	{
		float val_1_= FOC_Para_M[0].Motor_State_.shaft_velocity;
		float val_2_= FOC_Para_M[0].Motor_State_.current.q;
//	float val=Vel_Target;
    
    int temp_1_ = val_1_;    // 上位机需要整数参数，转换一下
		int temp_2_ = val_2_;    // 上位机需要整数参数，转换一下
    set_computer_value(SEND_FACT_CMD, CURVES_CH1, &temp_1_, 1); // 给通道 1 发送实际值
		set_computer_value(SEND_FACT_CMD, CURVES_CH2, &temp_2_, 1); // 给通道 1 发送实际值
//		printf("val,%f;act,%f\n",set_point,val);	
	}
}
